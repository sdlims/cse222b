...
def regularity_extraction(A, B):
    ...
    if (tplt_avg >= 7):
        template_incentive = 1.5
    elif (tplt_avg >= 4):
        template_incentive = 1.3
    elif (tplt_avg == 1):
        template_incentive = 0.01
    else:
        template_incentive = 1
    ...
    # print("Compression Out:", cmpr_out)

    return (size_penalty * cmpr_out * template_incentive) / tplt_avg
...
def compression_annealing(V):
    ...
    return (compression_eq / len(subgraph)) #Averages
...
def simulated_annealing(init_V, cooling_rate, start_temp, end_temp, max_iters, cutoff): #end_temp might be redundant
    ...
    while (temp > end_temp) and (iters < max_iters):
        ...
        if (delta_energy > 0.01) or (random.random() < math.exp(delta_energy / temp)):
...
def main():
    ...
    #pprint.pprint(simulated_annealing([1, 1, 1, 1, 1, 2, 2, 2, 2, 4, 3, 3, 3, 3, 3, 4, 4, 4, 4, 2], 0.9999, 100000, 1, 1000000, 1))
    

    print("Garbage:")
    print(compression_annealing([4, 2, 2, 4, 1, 2, 3, 3, 2, 1, 2, 1, 3, 3, 1, 4, 4, 4, 3, 1]))
    print("\n")
